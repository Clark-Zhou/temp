...
#pragma omp parallel for schedule(static)
    ...
    for (long long ii = 0; ii < static_cast<long long>(n); ++ii) {
        ...
        adjusted = (adjusted < 0.0f) ? 0.0f : adjusted;
        adjusted = (adjusted > 1.0f) ? 1.0f : adjusted;
        const float gray_enhanced =
        ...
        const float hdr_result = gray_enhanced * gain;
        const float compress_val = hdr_result / (1.0f + hdr_result);
        ...
        const float sin_x = compress_val * p0;
        const float sin_x2 = sin_x * sin_x;
        const float sin_approx =
            sin_x * (1.0f - sin_x2 * (1.0f / 6.0f) +
                     sin_x2 * sin_x2 * (1.0f / 120.0f));
        const float cos_x = r_val * p1;
        const float cos_x2 = cos_x * cos_x;
        const float cos_approx =
            1.0f - 0.5f * cos_x2 + cos_x2 * cos_x2 * (1.0f / 24.0f);
        const float noise = sin_approx * cos_approx;
        float final_val = (mask * 0.7f) + (noise * 0.3f);
        final_val = (final_val < 0.0f) ? 0.0f : final_val;
        final_val = (final_val > 1.0f) ? 1.0f : final_val;

        const float scaled = final_val * 4.0f;
        int idx = static_cast<int>(scaled);
        idx = (idx < 0) ? 0 : idx;
        idx = (idx > 4) ? 4 : idx;
        const float weight_pos = scaled - static_cast<float>(idx);
        const float weight =
            (idx < 4)
                ? (lut[idx] * (1.0f - weight_pos) + lut[idx + 1] * weight_pos)
                : lut[4];

        float value = compress_val * weight;
